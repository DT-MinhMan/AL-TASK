...
import {
  ArrowLeft, User, Tag, Calendar, Flag, MessageSquare, Paperclip,
  Clock, ExternalLink, ChevronDown, Send, Edit3, Trash2, Link2
} from "lucide-react";
...
interface Task {
  ...
  labels: string[]; dueDate?: string; projectId?: any; sprintId?: any;
  boardColumnId?: string; storyPoints?: number; epicId?: any;
...
const priorityColors: Record<string, { bg: string; text: string; label: string }> = {
  highest: { bg: "bg-red-100", text: "text-red-700", label: "Highest" },
  high: { bg: "bg-orange-100", text: "text-orange-700", label: "High" },
  medium: { bg: "bg-yellow-100", text: "text-yellow-700", label: "Medium" },
  low: { bg: "bg-blue-100", text: "text-blue-700", label: "Low" },
  lowest: { bg: "bg-gray-100", text: "text-gray-700", label: "Lowest" },
};
  ...
  const handleAddComment = async () => {
    ...
    try {
      const res = await api.post(apiRoutes.COMMENTS.BASE, {
        content: newComment, targetType: "task", targetId: taskId
      });
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      <div className="flex items-start gap-4 mb-6">
        <Link href="/projects" className="mt-1 p-1.5 hover:bg-gray-100 rounded-lg">
        ...
        <div className="flex-1">
          ...
          {task.projectId?.name && (
            <p className="text-sm text-gray-500 mt-1">
              in <Link href={`/projects/${task.projectId._id}`} className="text-indigo-600 hover:underline">{task.projectId.name}</Link>
            </p>
          )}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              {!editingDesc && (
                <button onClick={() => setEditingDesc(true)} className="text-sm text-indigo-600 hover:text-indigo-800 flex items-center gap-1">
            ...
            {editingDesc ? (
              ...
              <div
                ...
                className="text-sm text-gray-700 min-h-[60px] cursor-pointer hover:bg-gray-50 rounded-lg p-3 -m-3 transition-colors prose prose-sm max-w-none"
                dangerouslySetInnerHTML={{ __html: task.description || '<p class="text-gray-400 italic">Click to add description...</p>' }}
              />
            )}
          </div>
              ...
              {comments.map((comment) => (
                <div key={comment._id} className="flex gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-700 bg-gray-50 rounded-lg p-3" dangerouslySetInnerHTML={{ __html: comment.content }} />
                  </div>
                </div>
              ))}
              {comments.length === 0 && (
        ...
        <div className="space-y-4">
            ...
            <div className="space-y-4">
              <div>
                ...
                >
                  {["todo", "inprogress", "review", "done"].map((s) => (
                    <option key={s} value={s}>{s.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())}</option>
                  ))}
                </select>
              </div>
              <div>
              ...
              <div>
                ...
                {task.assigneeId ? (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-gray-800">{task.assigneeId.fullName || task.assigneeId.email}</span>
                  </div>
                ) : (
              ...
              {task.dueDate && (
                <div>
                  ...
                  <span className={`text-sm ${new Date(task.dueDate) < new Date() ? "text-red-600" : "text-gray-800"}`}>
              ...
              {task.storyPoints && (
                <div>
                  <label className="block text-xs font-medium text-gray-500 uppercase tracking-wider mb-1">Story Points</label>
                  <span className="text-sm text-gray-800">{task.storyPoints}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            ...
            <div className="flex flex-wrap gap-2">
              {task.labels.map((label) => (
                <span key={label} className="px-2 py-1 bg-indigo-100 text-indigo-700 rounded text-xs font-medium">{label}</span>
              ))}
            ...
            {task.attachments?.length > 0 ? (
              <div className="space-y-2">
                {task.attachments.map((att) => (
                  <a key={att.id} href={att.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 p-2 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                    <ExternalLink className="w-3.5 h-3.5 text-gray-400" />
                    <span className="text-sm text-gray-700 truncate">{att.name}</span>
                    <span className="text-xs text-gray-400 ml-auto">{(att.size / 1024).toFixed(1)} KB</span>
                  </a>
                ))}
              </div>
            ) : (
          ...
          {/* Activity */}
          <div className="bg-white rounded-xl border border-gray-200 p-5">
            <h3 className="font-semibold text-gray-900 mb-3 flex items-center gap-2"><Clock className="w-4 h-4" /> Activity</h3>
