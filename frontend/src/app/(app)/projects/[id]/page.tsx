...
interface Board { _id: string; name: string; columns: { id: string; name: string; order: number; wipLimit?: number }[]; }
interface Task { _id: string; key: string; title: string; type: string; priority: string; status: string; boardColumnId?: string; assigneeId?: any; labels: string[]; }

const priorityColors: Record<string, string> = {
  highest: "border-l-red-500", high: "border-l-orange-400", medium: "border-l-yellow-400",
  low: "border-l-blue-400", lowest: "border-l-gray-300"
};
...
export default function ProjectBoardPage() {
  ...
  const getTasksByColumn = useCallback((columnId: string) => {
    return tasks.filter(t => t.boardColumnId === columnId);
  }, [tasks]);
  ...
  const handleDrop = async (e: React.DragEvent, columnId: string) => {
    ...
    try {
      await api.put(apiRoutes.TASKS.MOVE(draggedTask._id), { boardId: board?._id, boardColumnId: columnId });
    } catch (e) {
      console.error(e);
      fetchData();
    }
  ...
  const handleCreateTask = async (columnId: string) => {
    ...
    try {
      const res = await api.post(apiRoutes.TASKS.BASE, {
        title, projectId, boardId: board?._id, boardColumnId: columnId,
        type: "task", priority: "medium", status: columnId
      });
    ...
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <Link href="/projects" className="p-1.5 hover:bg-gray-100 rounded-lg">
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className="px-2 py-0.5 text-xs bg-gray-100 text-gray-600 rounded-full">{project.key}</span>
            </div>
            {project.description && <p className="text-sm text-gray-500 mt-0.5">{project.description}</p>}
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Link href={`/projects/${projectId}/settings`} className="p-2 hover:bg-gray-100 rounded-lg">
          ...
          {board?.columns.sort((a, b) => a.order - b.order).map((column) => {
            ...
            return (
              <div
                ...
                className={`w-72 flex-shrink-0 bg-gray-100 rounded-xl flex flex-col transition-colors ${
                  dragOverColumn === column.id ? "ring-2 ring-indigo-400 ring-inset" : ""
                }`}
              >
                {/* Column Header */}
                <div className="flex items-center justify-between p-3">
                  ...
                  <button onClick={() => handleCreateTask(column.id)} className="p-1 hover:bg-gray-200 rounded">
                ...
                <div className="flex-1 overflow-y-auto px-2 space-y-2">
                  {columnTasks.map((task) => (
                    <div
                      ...
                      className={`group bg-white rounded-lg p-3 border-l-4 cursor-grab active:cursor-grabbing hover:shadow-md transition-shadow ${
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        <span className={`px-1.5 py-0.5 text-xs rounded text-white ${typeColors[task.type]}`}>
                          {task.type}
                        </span>
                      ...
                      <Link href={`/tasks/${task._id}`} className="block mt-2 text-xs text-indigo-600 hover:text-indigo-800">
                        View details →
                      </Link>
                    </div>
                  ))}
