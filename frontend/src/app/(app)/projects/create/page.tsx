...
export default function CreateProjectPage() {
  ...
  if (!currentWorkspace) {
    return <div className="p-6 text-center text-gray-500">Please select a workspace first</div>;
  }
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-6 space-y-6">
        <div>
          ...
          <input
            type="text"
            value={form.name}
            onChange={(e) => setForm({ ...form, name: e.target.value, key: e.target.value.toUpperCase().replace(/\s+/g, "").substring(0, 5) })}
        ...
        <div>
          ...
          <input
            ...
            className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 font-mono"
            placeholder="WEB"
            maxLength={10}
            required
          />
          <p className="mt-1 text-xs text-gray-400">Task keys will look like: {form.key || "KEY"}-1, {form.key || "KEY"}-2</p>
        </div>
        <div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              { value: "kanban", label: "Kanban", desc: "Visual board with columns", icon: LayoutGrid },
              ...
              <button
                ...
                className={`p-4 border-2 rounded-xl text-left transition-all ${
                  form.type === opt.value ? "border-indigo-500 bg-indigo-50" : "border-gray-200 hover:border-gray-300"
                }`}
              >
