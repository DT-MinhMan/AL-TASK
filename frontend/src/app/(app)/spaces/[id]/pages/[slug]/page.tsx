...
import { ArrowLeft, Edit3, Clock, User, Tag, Eye } from "lucide-react";

interface Page {
  _id: string; title: string; content: string; slug: string; status: string;
  authorId?: any; labels: string[]; version: number; viewCount: number;
  spaceId?: any; createdAt: string; updatedAt: string;
}
    ...
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          <Link href={`/spaces/${spaceId}`} className="p-1.5 hover:bg-gray-100 rounded-lg">
      ...
      <article className="bg-white rounded-xl border border-gray-200 p-8">
          ...
          {page.authorId && (
            <div className="flex items-center gap-1.5">
              <div className="w-5 h-5 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center text-xs font-bold">
                {page.authorId.fullName?.charAt(0) || "?"}
              </div>
              <span>{page.authorId.fullName || "Unknown"}</span>
            </div>
          )}
          <div className="flex items-center gap-1.5">
            <Clock className="w-4 h-4" />
            <span>Updated {new Date(page.updatedAt).toLocaleDateString("vi-VN")}</span>
          </div>
          <div className="flex items-center gap-1.5">
        ...
        <div
          ...
          dangerouslySetInnerHTML={{ __html: page.content || '<p class="text-gray-400 italic">No content yet. Click Edit to add content.</p>' }}
        />
      </article>
    </div>
  );
