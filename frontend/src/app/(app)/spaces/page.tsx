...
import { BookOpen, Plus, Globe, Lock, ChevronRight, FileText } from "lucide-react";

interface Space {
  _id: string; name: string; key: string; description?: string; type: string;
  ownerId?: any; members: any[]; _count?: { pages: number };
}
...
export default function SpacesPage() {
  ...
  useEffect(() => {
    ...
    const fetchSpaces = async () => {
      try {
        const res = await api.get(`${apiRoutes.SPACES.BASE}?workspaceId=${currentWorkspace._id}`);
        setSpaces(res.data);
    ...
    <div className="max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm">{spaces.length} spaces in {currentWorkspace.name}</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {spaces.map((space) => (
            <Link key={space._id} href={`/spaces/${space._id}`} className="group bg-white rounded-xl border border-gray-200 p-6 hover:shadow-md hover:border-indigo-200 transition-all">
              ...
              <h3 className="font-semibold text-gray-900 mb-1 group-hover:text-indigo-600 transition-colors">{space.name}</h3>
